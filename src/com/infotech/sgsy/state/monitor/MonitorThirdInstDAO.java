...
public class MonitorThirdInstDAO {
	...
	public boolean saveThirdInstallment(MonitorThirdInstActionForm helperForm,String createdBy,String createdOn) throws ClassNotFoundException {
		boolean flag = false;
		Persister persister = PersisterImpl.getPersister();
		Session session = persister.getSession();
		Transaction tx = session.beginTransaction();
		try {
			List<MonitorThirdInstVO> monitorThirdList = new ArrayList();
			
			for (int i = 0; i < helperForm.getProjectId().length; i++) {
		
				MonitorThirdInstVO monitorDetails = MasterThirdInstallmentDetailsFactory.getInstance("com.infotech.sgsy.state.monitor.MonitorThirdInstVO");
			...
			for (int i = 0; i < monitorThirdList.size(); i++) {
				MonitorThirdInstVO monitorDetails = MasterThirdInstallmentDetailsFactory.getInstance("com.infotech.sgsy.state.monitor.MonitorThirdInstVO");
				monitorDetails = (MonitorThirdInstVO) monitorThirdList.get(i);
