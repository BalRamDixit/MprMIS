...
public class MonitorSecondInstDAO {
	...
	public List<String> getPiaForProjectIds(String entityCode) {
	...
	public boolean saveSecoundInstallment(MonitorSecondInstActionForm helperForm,String createdBy,String createdOn) throws ClassNotFoundException {
		boolean flag = false;
		Persister persister = PersisterImpl.getPersister();
		Session session = persister.getSession();
		Transaction tx = session.beginTransaction();
		try {
			List<MonitorSecondInstVO> monitorSecoList = new ArrayList();
			
			for (int i = 0; i < helperForm.getProjectId().length; i++) {
		
				MonitorSecondInstVO monitorDetails = MasterSecoundInstallmentDetailsFactory.getInstance("com.infotech.sgsy.state.monitor.MonitorSecondInstVO");
			...
			//	monitorDetails.setEntityCode(entityCode);
				...
				monitorSecoList.add(monitorDetails);
		
			}
			for (int i = 0; i < monitorSecoList.size(); i++) {
				MonitorSecondInstVO monitorDetails = MasterSecoundInstallmentDetailsFactory.getInstance("com.infotech.sgsy.state.monitor.MonitorSecondInstVO");
				monitorDetails = (MonitorSecondInstVO) monitorSecoList.get(i);
