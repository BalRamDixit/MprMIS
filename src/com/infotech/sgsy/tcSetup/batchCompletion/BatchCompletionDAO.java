...
public class BatchCompletionDAO {


  
	public List<BatchCompletionVO> getdetails(List<ProjectDetailsVO> sanctionProjectList) throws HibernateException, Exception {
		List<BatchCompletionVO> stateList=new ArrayList<BatchCompletionVO>();
		...
		try{
			 ...
			 Criteria criteria3 = session.createCriteria(BatchCreationVO.class).add(Restrictions.in("tcID", criteria2.list()));
			 
			 Criteria criteria = session.createCriteria(BatchCompletionVO.class).add(Restrictions.in("batchID", criteria3.list()));

			 for(Object o : criteria.list()) {
				 stateList.add((BatchCompletionVO)o);
			}
		}
		catch (Exception e) {
			e.printStackTrace();
	...
	public List<BatchCreationVO> getBatchDetails(List<ProjectDetailsVO> sanctionProjectList) throws HibernateException, Exception {
		List<BatchCreationVO> batchList=new ArrayList<BatchCreationVO>();
		Persister persister = PersisterImpl.getPersister();
		Session session = persister.getSession();
		Transaction tx = session.beginTransaction();
		try{
			 ...
			 for(Object o : criteria.list()) {
				 batchList.add((BatchCreationVO)o);
			}
		}
		catch (Exception e) {
			e.printStackTrace();
...
/*	public List<BatchCompletionVO> getdetails(List<ProjectDetailsVO> sanctionProjectList) throws HibernateException, Exception {
		List<BatchCompletionVO> batch=new ArrayList<BatchCompletionVO>();
		Persister persister = PersisterImpl.getPersister();
		Session session = persister.getSession();
		Transaction tx = session.beginTransaction();
		try{
//			 Criteria criteria1 = session.createCriteria(ProjectDetailsVO.class).add(Restrictions.in("id", sanctionProjectList));
			 ...
			 Criteria criteria = session.createCriteria(BatchCompletionVO.class);
			 for(Object o : criteria.list()) {
				 BatchCompletionVO ob=(BatchCompletionVO)o;
				 batch.add(ob);
			}
		}
		catch (Exception e) {
		...
		System.out.println("man-batch"+batch);
		return batch;
	}*/
...
public boolean deleteRecordFromId(BatchCompletionVO helperVo) {
...
public boolean update(BatchCompletionVO helperVo) throws Exception {
	...
	public boolean save(BatchCompletionVO batchCreationVo) throws Exception {
	...
	public   Object getSelected__Trade__Of__Project_By_sector(String sectorId){
		...
		return result;
			...
			public List<BatchCreationVO> getBatchList(String batchId) {
			...
			public List <BatchCreationVO> getCraetedBatchDetails(List<ProjectDetailsVO> sanctionProjectList) {
	...
	public BatchCompletionVO getBatchCompletionById(String id) {
		BatchCompletionVO batchCompletionVO=null;
		Persister persister = PersisterImpl.getPersister();
		Session session = persister.getSession();
		Transaction tx = session.beginTransaction();
		try{
			Criteria criteria = session.createCriteria(BatchCompletionVO.class).add(Restrictions.eq("id", id));
			Object o = criteria.uniqueResult();
			batchCompletionVO=(BatchCompletionVO)o;
		}
		catch (Exception e) {
			e.printStackTrace();
			tx.rollback();
