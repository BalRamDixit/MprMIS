...
function save(){
	...
	if(status==true){
		document.forms[0].action="monitorFirstInstallment.do?methodName=save2"+"&"+tokenParameter+"="+tokenValue;
...
/* function clearField()
{
    ...
    /* function checkAmount(vv,idForCheck){
    	var abc=document.getElementById(idForCheck).value;
         var val1=$(vv).val();
         var tfp=parseInt(abc) * .50;
         var ss=val1.replace(/,/g , "");
        // alert("----"+ss+"-----------"+tfp); 
          if(parseInt(ss)>parseInt(tfp)){
           alert("Value should be Less than 50% of the Project Cost ");
           $(vv).val("");
           }
          else
          { 	}
...
<html:form action="login/monitorFirstInstallment">
			...
			<tr>
					<td align="center" class="pageHeader">Second Installment</td>					
                </tr>
			</table>
			<table width="100%">
			    <tr>
	               	<td>
		 				 ...
		 				 <a onclick="second()" class="tablinks active" >Second Installment</a>
			...
			<div style="overflow-x:scroll;overflow-y:hidden;">
			 ...
			 <tr>
                 ...
                 <th align="center">Date Issuance of E2<span class="text-error">*</span></th>
			...
			<logic:iterate id="mm" name="mm" indexId="index" scope="request">
				<tr>                               
					...
					<td><input type="hidden" name="projectId" value="${mm.projectId.id}"/><input type="text"  readonly="true"  value="${mm.projectId.projectID}"/><html:hidden property="id" value='${mm.id}'></html:hidden></td>      
					<td align="center"><span class="text-error"><html:text property="dateofIssuance"  readonly="true" styleId="d1${index}"  onchange="checkAllDates('${index}')" styleClass="datepicker input" value="${mm.dateofIssuance}" />  </span></td> 
					<td align="center"><span class="text-error"><html:text property="dateofRecommend" readonly="true" styleId="d2${index}" onchange="checkAllDates('${index}')" styleClass="datepicker input" value="${mm.dateofRecommend}" />  </span></td> 
					<td align="center"><span class="text-error"><html:text property="dateofReceipt"   readonly="true" styleId="d3${index}"  onchange="checkAllDates('${index}')" styleClass="datepicker input" value="${mm.dateofReceipt}" />  </span></td> 
					...
					<td align="center"><span class="text-error"><html:text styleId="d4${index}" readonly="true" styleClass="datepicker input" onchange="checkAllDates('${index}')" property="relDate"  value="${mm.relDate }" /><input type='hidden' value="${mm.reldateHidden}"></span></td>
					...
					<td align="center"><span class="text-error"><html:text styleId="utilisePercent" styleClass="input" onkeypress="return isNumberKey(event)" property="utilizationPercent" onkeyup="checkUtilizationPercent(this);" value='${mm.utilizationPercent}'/>  </span></td> 
