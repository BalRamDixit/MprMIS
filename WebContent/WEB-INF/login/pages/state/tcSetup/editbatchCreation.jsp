...
$(function() {	
  ...
  //  $("#batchCommencedDateId").datepicker({ dateFormat: 'dd-mm-yy' }); 
	...
	var date2 = new Date(batchStartDate);
	 $("#batchStartDateId").datepicker('option', 'minDate', date2);
		$("#batchStartDateId").datepicker('option', 'maxDate', date2);
		$("#ojtStartDateId").datepicker('option', 'minDate', date2); 
...
function getXMLHttpRequest() {
	...
	function save() {		
		...
		if(x=='true'){
 		 ...
 		 var id=document.getElementById("bactchide").value;		 
	  ...
	  {	
		...
		{		
		  ...
		  var status=window.confirm('Are You Sure You Want to Update ?');
			if(status==true){
			    document.forms[0].action="batchCreation.do?methodName=updateBatchDetails&id="+id+"&"+tokenParameter+"="+tokenValue;
  ...
  function isNumberKey(evt)
	{
	 ...
	 function showstartDate(projectid){	
			...
			xmlHttpRequest.send("projectid="+projectid);
			     ...
			     /* function compareDate(date){ 	
	 					...
	 					$("#batchFreezeDateId").datepicker('option', 'minDate', startDate);
					};       */
...
<jsp:useBean id="batchDetail" class="com.infotech.sgsy.tcSetup.batchCreation.BatchCreationVO" scope="request"/>		
					...
					<td><html:select property="tcID" styleId="tcIDId"   styleClass="form-control"  value="${myTcId}" onchange="javascript:showSector(this.value); ">	
					  	 		<logic:present name="tcList">
									<logic:iterate id="list" name="tcList">
										<html:option value="${list.id}">${list.tcID}(${list.district.districtName})</html:option>
									</logic:iterate>
								</logic:present>			  
					</html:select></td>					 
				</tr>	
					 		 ...
					 		 <html:option value="0">--SELECT--</html:option>
					 			<logic:present name="sectorlist">
									<logic:iterate id="list" name="sectorlist">
										<html:option value="${list.sectorId}">${list.sectorName}</html:option>
									</logic:iterate>
								</logic:present>
				...
				<td><html:select property="trade" styleId="tradeId"   styleClass="form-control"  value="${myTrade}" >	<!-- onchange=  "javascript:showbatchID();" -->
				 				...
				 				<logic:present name="tradelist">
									<logic:iterate id="list" name="tradelist"> 
										 <html:option value="${list.tradeId}">${list.tradeName}(${list.tradeCode})</html:option>
									</logic:iterate>
								</logic:present>
					 </html:select></td>
				</tr>
			...
			<th>BATCH ID<span class="text-error"></span></th>
				 <td><span class="text-error"><html:hidden property="id" styleId="bactchide" value="${batchDetail.id}"></html:hidden> <html:text property="batchID" styleId="batchIDId"   readonly="true" /> </span></td>
				</tr> 			 
				
				
					<tr>	
					<th>BATCH DURATION<span class="text-error"></span></th>
				 <td><span class="text-error"> <html:text property="batchDuration"  value="${batchDetail.batchDuration}" styleId="batchDurationId" onkeypress="return isNumberKey(event)" /> </span></td>
				<!-- </tr>  
				<tr> -->	
					<th>BATCH FREEZE DATE<span class="text-error"></span></th>
				 <td><span class="text-error"> <html:text property="batchFreezeDate" onchange="compareDate();" styleId="batchFreezeDateId" value="${batchDetail.batchFreezeDate}" /> </span></td>					 
				 </tr> 
				 
				<tr>  	
					<th>BATCH SIZE<span class="text-error"></span></th>
				 <td><span class="text-error"> <html:text property="batchSize"  onchange="javascript:checkData(this.value);" styleId="batchSizeId" value="${batchDetail.batchSize}" onkeypress="return isNumberKey(event)"  /> </span></td>					 
				</tr>	
					 
				<tr>	
					<th>BATCH START DATE<span class="text-error"></span></th>
				 <td><span class="text-error"> <html:text property="batchStartDate"   styleId="batchStartDateId" value="${batchDetail.batchStartDate}" /> </span></td>					 
				</tr> 
				
				<tr>
				  <th>OJT START DATE<span class="text-error"></span></th>
				 <td><span class="text-error"> <html:text property="ojtStartDate" value="${batchDetail.ojtStartDate}" styleId="ojtStartDateId" /> </span></td>
				</tr> 
...
<%-- 
			...
			<!-- 	</tr> 
				...
				<tr> -->	
				 ...
				 <td><span class="text-error"> <html:text property="resiSize"  styleId="resiSizeId" value="${batchDetail.resiSize}"  onkeypress="return isNumberKey(event)"  /> </span></td>					 
				</tr> 
				<tr>	
				<th>SC CANDIDATEDS COMMENCED<span class="text-error"></span></th>
				 <td><span class="text-error"> <html:text property="commenced_Sc"  value="${batchDetail.commenced_Sc}" styleId="commenced_ScId" onchange="javascript:checktotal(this.value);"  onkeypress="return isNumberKey(event)"  /> </span></td>					 
				<!-- </tr>				
				<tr> -->	
					<th>ST CANDIDATEDS COMMENCED<span class="text-error"></span></th>
				 <td><span class="text-error"> <html:text property="commenced_St"  value="${batchDetail.commenced_St}" styleId="commenced_StId" onchange="javascript:checktotal(this.value);"  onkeypress="return isNumberKey(event)"  /> </span></td>					 
				</tr>			
				
				<tr>	
					<th>OTHERS CANDIDATEDS COMMENCED <span class="text-error"></span></th>
				 <td><span class="text-error"> <html:text property="commenced_Others"  value="${batchDetail.commenced_Others}" styleId="commenced_OthersId" onchange="javascript:checktotal(this.value);"  onkeypress="return isNumberKey(event)"  /> </span></td>					 
				<!-- </tr>		
				...
				<tr>	
				 ...
				 <td><span class="text-error"> <html:text property="commenced_Women"   value="${batchDetail.commenced_Women}" styleId="commenced_WomenId"  onkeypress="return isNumberKey(event)"  /> </span></td>					 
				<!-- </tr>							
				
				<tr> -->	
					<th>MINORITY CANDIDATEDS COMMENCED <span class="text-error"></span></th>
				 <td><span class="text-error"> <html:text property="commenced_Minority"  value="${batchDetail.commenced_Minority}" styleId="commenced_MinorityId"  onkeypress="return isNumberKey(event)"  /> </span></td>					 
				</tr>				
				
				<tr>	
					<th>PWD CANDIDATEDS COMMENCED<span class="text-error"></span></th>
				 <td><span class="text-error"> <html:text property="commenced_Pwd"  value="${batchDetail.commenced_Pwd}" styleId="commenced_PwdId"  onkeypress="return isNumberKey(event)"  /> </span></td>					 
				</tr>			
