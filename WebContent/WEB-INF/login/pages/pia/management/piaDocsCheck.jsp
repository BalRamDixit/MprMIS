...
function showData() {
	document.forms[0].action = "verifyPiaAction.do?methodName=getPiaDetail&page=checkDocs&"+ tokenParameter + "=" + tokenValue;
...
function submitPIADetail(){
	if(validate()){
		var status = window.confirm("Do you want to submit.");
		if(status){
			document.forms[0].action = "verifyPiaAction.do?methodName=checkDocs&" + tokenParameter + "=" + tokenValue;
			document.forms[0].submit();	
		}
	}
}
...
function showRemark(){
	if (document.getElementById("piaApplicationStatus").value == "RR" ){
...
<table width="100%" align="center" style="padding-down: 7px;" class="piaLogTbl">
	...
	<th width="50%"><label class="label-info">Applicant Organisation List for Document Checking</label></th>
	<td align="center">
...
<tr>
<td>
		<!-- <bean:write name="piaDetail" property="enrolmentNumber"/> -->
...
<td>
	...
	<a href="javascript:showFiles('${piaDetail.tanFileName}');" >View</a><br/>		
	</td>
...
<th width="30%">Applicant Organisation Application Status</th>
	<td>
		<html:select property="piaApplicationStatus" styleId="piaApplicationStatus" onchange="javascript:showRemark();">
			<html:option value="">Select</html:option>
			<html:option value="C">Documents checked and found proper</html:option>
			<%-- <html:option value="P">Documents checked and found incomplete</html:option>	 --%>
			<html:option value="RR">Recommended for Rejection</html:option>			
		</html:select>
	</td>
</tr>
...
<div align="center" style="padding-top: 15px; padding-bottom: 10px;"> 
	<html:button  styleClass="primaryBtn" property="" onclick="submitPIADetail()">Submit</html:button>
