...
<%-- 						<td>${list['editId']}</td> --%>
		...
		<tr>
			...
			<div id="insertNewDataDiv">
				<table width="60%" >
					<tr>
						<th colspan="2">Select <c:if test="${RoleId eq '5'}">either Role or </c:if>User to Assign Module permission</th>
				   </tr>
				   <tr>
				...
				<table width="100%" style="text-align: center;border-style: solid;border-color: gray"  class="tableRow">
				   ...
				   <logic:present name="formModulesList">
				   		<c:forEach items="${formModulesList}" var="formModules">
				   			<c:forEach items="${formModules.value}" var="project">
...
<%-- 					<input name="Button" type="button" class="button" value="<bean:message  key="button.clear"/>" onclick="clearField()"/>  --%>
			...
			<div id="editOldDataDiv">
				...
				<table width="60%" >
					...
					<tr>
						<th colspan="2">Select <c:if test="${RoleId eq '5'}">either Role or </c:if>User to Add Module permission</th>
				   </tr>
				   <tr>
				   		<c:if test="${RoleId eq '5'}">
				...
				<table width="100%" style="text-align: center;border-style: solid;border-color: gray" class="tableRow">
				   ...
				   <logic:present name="formModulesListForEdit">
						<c:forEach items="${formModulesListForEdit}" var="formModules">
				   			<c:forEach items="${formModules.value}" var="project">
								<c:if test="${project.checkForMasterModule eq true}">
									<tr style="background-color: green">
								</c:if>
