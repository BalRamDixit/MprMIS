...
<table width="100%" align="center" style="padding-down: 7px;" class="bordered">	  	
	  	...
	  	<logic:notEmpty name="piaDetail" property="remark">
			<label class="label-important" >Faulty applications returned:&nbsp;
			 ${piaDetail.remark}</label>
		</logic:notEmpty>	
...
<table width="100%" align="center" style="padding-down: 7px;" class="bordered">	  	
		...
		<br/>
		(<c:if test="${piaDetail.piaStatus == 'P'}" ><span class="text-info">Pending</span></c:if>
		<c:if test="${piaDetail.piaStatus == 'C'}" ><span class="text-primary">Under Verification</span></c:if>
		...
		<c:if test="${piaDetail.piaStatus == 'RR'}" ><span class="text-error">Under Verification</span></c:if>
...
<td>
	<%-- <c:if test="${piaDetail.enrolmentFileName != null}">
	    	<strong><b>Unique Id</b></strong>:&nbsp;<bean:write name="piaDetail" property="enrolmentNumber"/>&emsp;
	<a href="javascript:showFiles('${piaDetail.enrolmentFileName }');" >View</a>
	    </c:if> --%>
	  <c:choose>
    <c:when test="${piaDetail.enrolmentFileName != null}">
       <strong><b>Unique Id</b></strong>:&nbsp;<bean:write name="piaDetail" property="enrolmentNumber"/>&emsp;
	<a href="javascript:showFiles('${piaDetail.enrolmentFileName }');" >View</a>
    </c:when>
   
    <c:otherwise>
        <strong><b>Unique Id</b></strong>: Not Available
    </c:otherwise>
</c:choose>  
...
<table width="100%" align="center" class="bordered" >
	...
	<tr><th colspan="4">Details of the Owners/ Directors </th></tr>	
	
	<tr>
		<th><strong>Name</strong></th>		
		<th><c:if test="${(piaDetail.categoryCode != 3) && (piaDetail.categoryCode != 4)}">
	...
	<td><span class="text-inverse"><bean:write name="member" property="contact"/></span></td>
	</tr>
			
	<tr>
		<th >PAN No.</th>
		<th >Voter Id / Aadhaar</th>
		<th >Passport / Driving License</th>
		<th></th>
	</tr>
	    ...
	    <tr>
	       <td>	  
	    	...
	    	<td><strong>Name</strong>:&nbsp;<bean:write name="member" property="memberName"/><br/>
				...
				<strong>Occupation:</strong>&nbsp;<bean:write name="member" property="occupation"/><br/>
				<strong>Voter Id / Aadhaar:</strong>&nbsp;<bean:write name="member" property="aadharVoterNo"/><br/>
				
			</td>	
		    <td>
		    <strong>Residence Address:</strong><br/><bean:write name="member" property="address"/><br/>				
			<strong>Contact No.:</strong>&nbsp;<bean:write name="member" property="contact"/><br/>	
			<strong>PAN No.:</strong>&nbsp;<bean:write name="member" property="pan"/><br/>	
			<strong>Email Id:</strong>&nbsp;<bean:write name="member" property="email"/><br/>
			<strong>Passport / Driving License:</strong>&nbsp;<bean:write name="member" property="passportDrivingNo"/>			
			</td>
