...
{
window.open(url,"","menubar=0,resizable=1,status=1,width=200,height=200,left=0,top=0 ,scrollbars=yes" );
}	
	
function showPopBig(url)
{
window.open(url,"","menubar=0,resizable=1,status=1,width=800,height=600,left=0,top=0 ,scrollbars=yes" );
}	
...
/*function show(sel1,sel2)
{
		 ...
		 for(var i=0; i<sel1.length; i++)
		 {
            ...
            {					
				for (var k=0; k<j; k++)
				{
					if (sel2.options[k].selected != sel1.options[i].selected && sel2.options[k].value != sel1.options[i].value)
					{
						sel2.options[j++]=new Option(sel1.options[i].text,sel1.options[i].value,false,true);
						sel1.options[i--]=null;
					}
...
function trim(str)
  ...
  function isNumber(evt)
    ...
    // Email Validation Added by CVAS2273 (24th-NOV-2009)
